...
export function SortOrderSelector(): JSX.Element | null {
  const { selectedSortOrder, setSelectedSortOder } = useGameQueryStore(
    (selector) => ({
      selectedSortOrder: selector.gameQuery.sortOrder,
      setSelectedSortOder: selector.setSortOrder,
    }),
  );
  const selectedSortOrderObject = sortOrderObjects.find(
    (sortOrderObject: SortOrderObject): boolean => {
  ...
  return (
    <Menu>
      ...
      <MenuList>
        {sortOrderObjects.map(
          (sortOrderObject: SortOrderObject): JSX.Element => {
            return (
              <MenuItem
